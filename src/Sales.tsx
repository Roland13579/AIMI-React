...
import { Modal, Button, Form, Badge, Toast, ToastContainer } from "react-bootstrap";
...
const Sales: React.FC = () => {
  ...
  const refreshForecastData = () => {
    ...
    fetch("http://127.0.0.1:5000/refresh-forecast-data", {
      method: "POST",
    })
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        if (data.status === "success") {
          ...
          setToastMessage(`Forecast data refreshed successfully. ${data.items_processed} items processed.`);
        } else {
